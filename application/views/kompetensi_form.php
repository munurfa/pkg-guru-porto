...
// echo $this->uri->segment(2);
                    foreach (range(1, 14) as $k) {
                        $kom['km'.$k] = 'kompetensi '.$k;
                    }
                        // $ubah = ($this->uri->segment(2)=="update") ? ['readonly'] : '' ;
                        $pilih = ($this->input->post('kode_kompetensi')) ? $this->input->post('kode_kompetensi') : $kode_kompetensi ;

                       echo form_dropdown('kode_kompetensi', $kom, $pilih, array('class' => 'form-control' ));
                    ?>
